...
--   [1] Added field-level comment: annotations tracing each field to a business question ID (B1-B24)
...
AS $$
  ...
  comment: "Customer profile, preferences, and cross-domain activity snapshot (B1, B4, B5, B12, B24)"
  joins:
    - name: ctx
  ...
  dimensions:
    ...
    - name: Preferred Contact Channel
      ...
      comment: "Channel banker should use when contacting the customer — phone, email, branch (B5)"
    ...
    - name: Satisfaction Score
      ...
      comment: "Customer satisfaction rating — use to assess dissatisfaction trend (B4)"
    ...
    - name: Latest Application Stage
      ...
      comment: "Current pipeline stage, e.g. DOCUMENT_REVIEW, ASSESSMENT (B16)"
...
AS $$
  ...
  comment: "Current application status, stage, and action flags (B6, B9, B14-B17, B22, B23)"
  dimensions:
    - name: Application ID
      expr: application_id
    ...
    - name: Recorded Reason
      ...
      comment: "Reason recorded when an application was returned or status changed — B10"
  measures:
...
AS $$
  ...
  comment: "Document checklist and status per application (B7, B11, B13, B20, B21)"
  dimensions:
    - name: Application ID
      expr: application_id
    ...
    - name: Rejection Reason
      ...
      comment: "Reason the document was rejected — e.g. ILLEGIBLE, INCORRECT_TYPE, EXPIRED_ON_RECEIPT (B21)"
    - name: Requested At
      expr: requested_at
...
AS $$
  ...
  comment: "Full audit trail of application state changes (B8, B10, B18, B19)"
  dimensions:
    - name: Application ID
      expr: application_id
    ...
    - name: Recorded Reason
      expr: recorded_reason
      comment: "Reason recorded at this event — critical for B10: 'Why was this application returned?'"
  measures:
    - name: Event Count
      expr: COUNT(1)
...
AS $$
  ...
  comment: "Open and resolved service cases per customer (B3, B4, B13)"
  dimensions:
    - name: Case ID
      expr: case_id
    ...
    - name: Organisation ID
      ...
      comment: "Organisation for business cases — use in mv_banker_business context"
    - name: Application ID
      expr: application_id
...
AS $$
  ...
  comment: "Customer interaction and service activity log (B2)"
  dimensions:
    - name: Customer ID
      expr: customer_id
...
AS $$
  version: 1.1
  source: "`0-ai-trust`.gold.fact_application_current"
  comment: "Approved next-action wording from stage-action policy (B22, B24, C13 via customer view). Data pending — dim_stage_action_policy currently returns POLICY_NOT_CONFIGURED."
  joins:
    - name: policy
